...
class TrainLoop:
    def __init__( 
        ...
        *,
        ...
        self.data = iter(data)
        #self.step = 0 #to ensure every sequence only constitute a single location
        ...
        self.n = 1 # the multiple of time_len
        self.flag = False
        self.ema_rate = (
            [ema_rate]
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            checkpoint = th.load(resume_checkpoint, map_location=self.device)
    ...
    def _load_ema_parameters(self, rate):
        ...
        if ema_checkpoint:
            ...
            ema_checkpoint= th.load(ema_checkpoint, map_location=self.device)

            ema_params = self._state_dict_to_master_params(ema_checkpoint)
    ...
    def run_loop(self) :
        ...
        def should_skip(start_idx, end_idx):
            start_block = start_idx // self.time_len
            end_block = end_idx // self.time_len
        ...
        while (
            ...
            end_idx = start_idx + self.seq_len - 1 + self.batch_size - 1
            if should_skip(start_idx, end_idx):
                self.step += 1
                self.flag = True
                print('skipped:', start_idx, end_idx)
                continue
            if self.flag==True:
                self.n += 1
                self.flag=False
    ...
    def forward_backward(self, batch):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            loss += (losses['res_loss']).mean()
            log_loss_dict(self.diffusion, t, {k: (v if k == 'res_loss' else v * weights) for k, v in losses.items()})            
